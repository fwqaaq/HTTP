...
fn get_mac_through_arp(interface: NetworkInterface, target_ip: Ipv4Addr) -> MacAddr {
    ...
    arp_packet.set_operation(ArpOperations::Request); // request = 1, reply = 2
    arp_packet.set_sender_hw_addr(interface.mac.unwrap()); // mac address of the sender
    arp_packet.set_sender_proto_addr(source_ip); // ip address of the sender
    arp_packet.set_target_hw_addr(MacAddr::zero()); // 0 for broadcast
    arp_packet.set_target_proto_addr(target_ip); // ip address of the target
    ...
    loop {
            ...
            && arp.get_target_hw_addr() == interface.mac.unwrap()
        {
            ...
            println!("Sender MAC address: {:#?}", arp);
